...
public sealed class UdmdPromptPage : ContentPage
{
    ...
    private UdmdPromptPage(long cardId, IReadOnlyList<UdmdFieldPromptModel> fields)
    {
        _cardId = cardId;
        _fields = fields;
        Title = "Metadata";
    ...
    public static async Task<UdmdPromptResult> PromptForCardAsync(Page owner, IUdmdService udmd, long cardId)
    {
        ...
        var page = new UdmdPromptPage(cardId, fields);
    ...
    private async Task CaptureImageAsync(UdmdFieldPromptModel field, ImageInputView control)
    {
        try
        {
            ...
            var fileName = UdmdImageFileStore.CreateFileName(field.Config, DateTime.Now, photo.FileName);
        ...
        {
            await DisplayAlert("Camera permission", "Camera permission is required to capture this metadata image.", "OK");
        }
        catch (Exception ex)
        {
            await DisplayAlert("Image capture failed", ex.Message, "OK");
        }
    }
    ...
    private async Task TrySaveAsync()
    {
        ...
        foreach (var field in _fields)
        {
            ...
            {
                await DisplayAlert("Required field", $"{field.Config.FieldName} is required.", "OK");
                return;
            }
            ...
            {
                await DisplayAlert("Invalid number", $"{field.Config.FieldName} must be a number.", "OK");
                return;
            }
