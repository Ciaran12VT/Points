...
namespace Points.ViewModels.Shortcuts;
...
public sealed class ShortcutDetailsViewModel : ObservableObject
{
    ...
    private ShortcutGroupModel? _selectedGroup;
    ...
    { 
        ...
        set
        {
            ...
            if (_selectedGroup != null)
                ApplyGroupDetails(_selectedGroup);
        }
    }
    ...
    public Command ClearGroupCommand { get; }
        ...
        TargetCardType defaultType = TargetCardType.MainQuest)
    {
        ...
        _existingGroupNames = _existingGroups.Select(x => x.Name).ToList();
    ...
    {
        var initial = string.IsNullOrWhiteSpace(GroupName)
            ? Array.Empty<string>()
            : new[] { GroupName.Trim() };
        ...
        var selectedName = result.FirstOrDefault()?.Trim() ?? "";
        _selectedGroup = _existingGroups.Find(g =>
            g.Name.Equals(selectedName, StringComparison.OrdinalIgnoreCase));
    ...
    private void ApplyGroupDetails(ShortcutGroupModel group)
    {
        GroupColor = group.Color;
        GroupOrderText = group.ShortcutGroupOrder.ToString(CultureInfo.InvariantCulture);
    }
