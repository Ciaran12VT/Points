...
namespace Points.ViewModels.Budgets
{
    public class BudgetDetailsViewModel : ObservableObject
    {
            ...
            ITimeZoneService timeZoneService)
        {
            ...
            var start = _model.StartDate == default ? _clock.LocalNow : _model.StartDate;
        ...
        private async Task OpenTransactionLogAsync()
        {
            ...
            await _navigation.PushAsync(new Points.Views.Budgets.BudgetTransactionLogPage(
        ...
        private async Task EditUdmdAsync()
        {
            if (_model.CardID <= 0)
            {
                await _dialogs.DisplayAlertAsync(
                    "Save required",
                    "Please save the card before configuring metadata fields.",
                    "OK");
                return;
            }

            await _navigation.PushAsync(new Points.Views.Udmd.UdmdConfigPage(_model.CardID, _udmd));
        }
